...
public class DFSConfigKeys extends CommonConfigurationKeys {
  ...
  // editlog事务数量，达到这个事务数量后，SecondaryNameNode或者StandbyNameNode会触发线checkpoint操作
  ...
  /**
   * 确定active namenode何时滚动自己的编辑日志。实际的阈值(编辑的数量)是通过将这个值乘以dfs.namenode.checkpoint.txns来确定的。
   * 这可以防止非常大的编辑文件积累在active namenode上，这可能会在namenode启动期间导致超时并造成管理上的麻烦。
   * 当standby或secondary namenode无法将编辑日志滚到正常的检查点阈值时，此行为将作为故障安全措施。
   */
  ...
  // namenode扫描lazypersist文件的时间间隔，寻找缺少块的文件，并将它们从namespace中取消链接。当设置为负值时，则禁用此属性。
  ...
  public static final String  DFS_JOURNALNODE_EDITS_DIR_KEY = "dfs.journalnode.edits.dir"; // JournalNode editlog存储路径
  public static final String  DFS_JOURNALNODE_EDITS_DIR_DEFAULT = "/tmp/hadoop/dfs/journalnode/";
  public static final String  DFS_JOURNALNODE_RPC_ADDRESS_KEY = "dfs.journalnode.rpc-address"; // JournalNode RPC地址
